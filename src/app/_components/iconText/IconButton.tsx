...
const IconButton: React.FC<IconButtonProps> = ({ icon, name, number }) => {
  return (
    <div className="group relative text-primaryDark cursor-pointer rounded-full p-2 box-border grid place-items-center border-2 border-primaryDark w-[100px] overflow-hidden">
      <FontAwesomeIcon icon={icon} className="w-[30px] relative z-10" />
      {number && (
        <span className="absolute -top-4 -right-8 translate-x-[-50%] bg-helper w-8 h-8 text-sm rounded-full grid place-items-center text-primaryDark p-[0.1rem]  z-10">
          {number}
        </span>
      )}
      <h2 className="text-primaryDark relative z-10">{name}</h2>
