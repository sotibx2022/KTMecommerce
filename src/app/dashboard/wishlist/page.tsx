...
const WishListItemsPage = () => {
  ...
  const { wishListItems, initialized } = useSelector((state: ReduxState) => state.wishList);
  const isLoading = userDetailsLoading || !initialized;
        ...
        <>
          <div className="wishlistPageHeader mb-4">
            <Button onClick={() => setVisibleComponent('publicWishlist')}>
              <span className="mr-2"><UnlockIcon /></span> Public
            </Button>
          </div>
          <div className="grid grid-cols-1 gap-y-10 gap-x-6 sm:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 xl:gap-x-8">
